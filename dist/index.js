...
const app_validation = async (worskpace) => {
    const pendingDhtOps = getValidationLimboDhtOps(worskpace.state, ValidationLimboStatus.SysValidated);
...
const callZomeFn = (zomeName, fnName, payload, provenance, cap) => async (worskpace) => {
    ...
    if (getTipOfChain(contextState) !== currentHeader) {
        ...
        while (i < contextState.sourceChain.length) {
            ...
            const depsMissing = await sys_validate_element(element, { ...worskpace, state: contextState }, worskpace.p2p);
            if (depsMissing)
